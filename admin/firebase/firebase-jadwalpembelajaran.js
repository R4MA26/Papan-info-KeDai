...
var jadwalRef = db.ref("pembelajaran");

//referensi untuk tambah data 
var addjadwalRef = db.ref("pembelajaran/" + id);
...
function dataBerhasil(data) {
    ...
    data.forEach(function (cetak) {
        tabel_jadwalpembelajaran +=
            ...
            '<td><button class="btn btn-sm btn-warning" data-toggle="modal" data-target="#bs-example-modal-lg-2" onclick="editJadwalPembelajaran(\'' + cetak.val().id + '\')">Edit</button> <button class="btn btn-sm btn-danger" onclick="deleteJadwalPembelajaran(\'' + cetak.val().id + "')\">Hapus</button></td>" +
            "</tr>";
        nomor++;
    });
...
function editJadwalPembelajaran(id) {
    ...
    var query = db.ref('pembelajaran/' + id);
    ...
    document.getElementById("form-jadwalpembelajaran-edit").addEventListener("submit", e => {
